...
contract BancorMarketMaker is EtherTokenConstant, IsContract, AragonApp {
    ...
    */
    ...
    function _createBuyOrder(address _buyer, address _collateralToken, uint256 _value) internal {
        ...
        _transfer(_buyer, address(this), _collateralToken, valueAfterFee);
        if (fee > 0)
    ...
    function _createSellOrder(address _seller, address _collateralToken, uint256 _amount) internal {
        ...
        // Tokens are moved into this contrat's account until they can be matched or burned
        _transfer(_seller, address(this), _collateralToken, _amount);
    ...
    function _clearMatching(address collateralToken) internal {
        ...
        uint256 poolBalance = controller.balanceOf(address(reserve), collateralToken);
        uint256 totalSupply = token.totalSupply();

        // the static price is the current exact price in collateral
        // per token according to the initial state of the batch
        uint256 staticPrice = getPricePPM(collateralToken, totalSupply, poolBalance);
        ...
        if (resultOfSell >= batch.totalBuySpend) {
            ...
            uint256 remainingSellReturn = getSell(collateralToken, totalSupply, poolBalance, remainingSell);
            ...
            // result in match orders + bonding curve sell
            // match order tokens already held by (this)
            // remaining tokens to be burned are difference between totalBuyReturn totalSellSpend
            tokenManager.burn(address(this), remainingSell);

            // collateral amount that resulted from the curve needs to be held by (here)
            _transfer(address(reserve), address(this), _collateralToken, remainingSellReturn);
        ...
        // more buys than sells
            ...
            // order was just smaller than originally thought).
            uint256 remainingBuyReturn = getBuy(collateralToken, totalSupply, poolBalance, remainingBuy);
            ...
            // result in match orders + bonding curve buy
            // match order tokens are already held by (this)
            // remaining tokens to be minted are difference between totalBuyReturn and totalSellSpend
            tokenManager.mint(address(this), remainingBuyReturn);

            // collateral amount that went into the curve needs to be put into the reserve
            _transfer(address(this), address(reserve), _collateralToken, remainingBuy);
        }
    }
    ...
    function _claimBuy(address _buyer, address _collateralToken, uint256 _batchId) internal {
        ...
        if (buyReturn > 0) {
            ...
            _transfer(address(this), _buyer, _collateralToken, buyReturn);
        }
    ...
    function _claimSell(address _seller, address _collateralToken, uint256 _batchId) internal {
        ...
        if (amountAfterFee > 0) {
            ...
            _transfer(address(this), _seller, _collateralToken, amountAfterFee);
        }
        if (fee > 0) {
            // reserve.transfer(_collateralToken, beneficiary, fee);
            _transfer(address(this), beneficiary, _collateralToken, fee);
        }
