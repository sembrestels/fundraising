...
function renderEntries(entries, { renderEntry, renderEntryActions, renderEntryChild }) {
  return entries.map(entry => {
    ...
    return {
      ...
      actions: renderEntryActions ? renderEntryActions(values, index, extraData) : null,
      children: entryChildrenFromChild(renderEntryChild ? renderEntryChild(values, index, extraData) : null),
    }
  })
}
...
function DataView({
  ...
  return (
    <Box padding={false}>
      ...
      {layoutName !== 'large' ? (
        <ListView entries={renderedEntries} fields={renderedFields} hasAnyChild={hasAnyChild} />
      ) : (
        <TableView entries={renderedEntries} fields={renderedFields} hasAnyActions={hasAnyActions} hasAnyChild={hasAnyChild} rowHeight={tableRowHeight} />
      )}
