...
import { GU, noop, useTheme, Box, Pagination, useLayout } from '@aragon/ui'
...
function renderFields(fields) {
  return fields.map((fieldFromProps, index, fields) => {
    ...
    const field = fieldFromProps && fieldFromProps.label ? fieldFromProps : { label: fieldFromProps }
...
function renderEntries(entries, { fields, renderEntry, renderEntryActions, renderEntryChild }) {
  return entries.map(entry => {
    ...
    return {
      index,
      entryNodes,
      actions: renderEntryActions ? renderEntryActions(value, index, { selected }) : null,
      children: entryChildrenFromChild(renderEntryChild ? renderEntryChild(value, index, { selected }) : null),
      selected,
    }
  })
}
...
function useSelection(entries, onSelectEntries) {
  ...
  const selectAll = useCallback(() => {
    setSelectedIndexes(selectedIndexes.length === 0 ? entries.map((_, index) => index) : [])
  }, [entries, selectedIndexes])

  useEffect(() => {
    if (onSelectEntries) {
      onSelectEntries(entries.filter((entry, index) => selectedIndexes.includes(index)), selectedIndexes)
    }
...
const DataView = React.memo(function DataView({
  ...
  if (renderEntryChild && onSelectEntries) {
    throw new Error('A DataView cannot be made selectable if it has entry children. ' + 'Please use either renderEntryChild or onSelectEntries.')
  }

  const theme = useTheme()
  const { name: layoutName } = useLayout()
  const { allSelected, selectAll, toggleEntry, selectedIndexes } = useSelection(entries, onSelectEntries)
    ...
    <Box padding={false}>
      ...
      {pages > 1 && (
        <div
          ...
          `}
        >
          <Pagination pages={pages} selected={currentPage} onChange={onPageChange} touchMode={layoutName === 'small'} />
        </div>
      )}
    </Box>
  )
