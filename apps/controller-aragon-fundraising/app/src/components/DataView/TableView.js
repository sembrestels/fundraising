...
function TableView({ renderHeader, rowHeight, hasAnyActions, hasAnyChild, fields, entries }) {
  ...
  return (
    ...
    >
      <thead>
        <HeadRow fields={fields} hasAnyActions={hasAnyActions} hasAnyChild={hasAnyChild} />
      </thead>
      <tbody>
        {entries.map((entry, index) => {
          ...
          return (
            <React.Fragment key={index}>
              ...
              />
              {hasChildren && <EntryChildren entry={entry} opened={entry.index === opened} rowHeight={rowHeight} />}
            </React.Fragment>
          )
        })}
      </tbody>
...
function EntryRow({ entry, fields, hasAnyActions, hasAnyChild, hasChildren, opened, rowHeight, toggleEntry }) {
  const theme = useTheme()

  const cells = entry.values.map((content, index) => [content, fields[index].align, false])

  if (hasAnyChild) {
    cells.unshift([hasChildren && <ToggleCell index={entry.index} opened={opened} onToggle={toggleEntry} />, 'start', true])
  }
